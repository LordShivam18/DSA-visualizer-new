...
import Controls from "../../../components/binary-search/find-peak-element/Controls";
...
import {
  generateTrace,
  type PeakSearchTraceStep,
} from "../../../components/binary-search/find-peak-element/generateTrace";

const defaultInput = "1 2 3 1";

const presets = [
  { name: "Example 1", value: "1 2 3 1" },
  { name: "Example 2", value: "1 2 1 3 5 6 4" },
  { name: "Edge Peak", value: "9 7 5 3 1" },
] as const;

export default function FindPeakElementPage() {
  const [input, setInput] = useState(defaultInput);
  const [trace, setTrace] = useState<PeakSearchTraceStep[]>(() =>
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6">
        ...
        <header className="mx-auto flex w-full max-w-4xl flex-col items-center gap-4 text-center">
          <p className="text-xs uppercase tracking-[0.34em] text-slate-500">
            Binary Search / Peaks / Slope Decisions
          </p>
          <h1 className="text-4xl font-semibold tracking-tight md:text-6xl">
            <span className="text-cyan-400 text-glow-cyan">Find Peak Element</span>
          </h1>
          <p className="max-w-3xl text-sm leading-7 text-slate-400 md:text-base">
            Instead of checking every hill, this visualizer uses one local slope
            comparison to prove which side still contains a peak.
          </p>
        </header>
        ...
        <div className="mx-auto w-full max-w-4xl">
          <div className="glass-card p-5">
            ...
            <label className="text-xs font-medium text-slate-400">Array values</label>
            <textarea
              value={input}
              onChange={(event) => setInput(event.target.value)}
              className="input-field mt-2 min-h-[150px] resize-y"
              placeholder={"Use values like 1 2 3 1 or [1,2,1,3,5,6,4]"}
            />
        ...
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,0.92fr)]">
          <section className="space-y-5">
            ...
            <PeakSearchVisualizer step={step} />
        ...
        <div className="mx-auto w-full max-w-4xl">
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-400">
              ...
              <span>
                Input length:{" "}
                <span className="font-mono text-slate-200">{step.state.nums.length}</span>
              </span>
            </div>
          </div>
        </div>
