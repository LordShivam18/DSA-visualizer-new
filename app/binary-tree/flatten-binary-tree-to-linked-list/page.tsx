"use client";

import TraceLessonPage from "@/components/academy/TraceLessonPage";
import { toLessonTrace } from "@/components/academy/traceAdapters";

import CodePanel from "../../../components/binary-tree/flatten-binary-tree-to-linked-list/CodePanel";
import MicroscopeView from "../../../components/binary-tree/flatten-binary-tree-to-linked-list/MicroscopeView";
import FlattenWorkbench from "../../../components/binary-tree/flatten-binary-tree-to-linked-list/FlattenWorkbench";
import TracePanel from "../../../components/binary-tree/flatten-binary-tree-to-linked-list/TracePanel";
import { generateTrace } from "../../../components/binary-tree/flatten-binary-tree-to-linked-list/generateTrace";

const defaultInputs = {
  root: "1,2,5,3,4,null,6"
};

const presets = [
  { name: "Default", summary: "Trace baseline", values: defaultInputs },
];

function generateLessonTrace(values: typeof defaultInputs) {
  return toLessonTrace(generateTrace(values.root));
}

export default function FlattenBinaryTreeToLinkedListPage() {
  return (
    <TraceLessonPage
      variant="dark"
      categoryHref="/binary-tree"
      categoryLabel="Binary Tree"
      taxonomy="Binary Tree / Trace-driven lesson"
      title="Flatten Binary Tree to Linked List"
      difficulty="Medium"
      description="Trace the Flatten Binary Tree to Linked List algorithm from parsed input through each decision point."
      complexity="Trace generated from pure algorithm state"
      defaultInputs={defaultInputs}
      inputFields={[
        { id: "root", label: "root", placeholder: "1,2,5,3,4,null,6", }
      ]}
      presets={presets}
      generateTrace={generateLessonTrace}
      renderVisualization={({ step }) => <FlattenWorkbench step={step} />}
      renderMicroscope={({ step, teachingMode }) => (
        <MicroscopeView step={step} mode={teachingMode} />
      )}
      renderTracePanel={({ step }) => <TracePanel step={step} />}
      renderCodePanel={({ step }) => <CodePanel step={step} />}
    />
  );
}
