...
import Controls from "../../../components/heap/ipo/Controls";
...
import {
  ...
  generateTrace,
  type IPOTraceStep,
} from "../../../components/heap/ipo/generateTrace";

const defaultK = "2";
const defaultCapital = "0";
const defaultProfits = "1, 2, 3";
const defaultRequirements = "0, 1, 1";
...
export default function IPOPage() {
  ...
  const [capitalInput, setCapitalInput] = useState(defaultCapital);
  const [profitsInput, setProfitsInput] = useState(defaultProfits);
  const [requirementsInput, setRequirementsInput] = useState(defaultRequirements);
  const [trace, setTrace] = useState<IPOTraceStep[]>(() =>
    generateTrace(defaultK, defaultCapital, defaultProfits, defaultRequirements)
  );
  ...
  function runWithValues(
    ...
    setKInput(nextK);
    setCapitalInput(nextCapital);
    setProfitsInput(nextProfits);
    setRequirementsInput(nextRequirements);
    setTrace(generateTrace(nextK, nextCapital, nextProfits, nextRequirements));
    setCursor(0);
  }

  const liveAnswer = step.done ? step.state.answer : step.state.currentCapital;
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6">
        ...
        <header className="mx-auto flex w-full max-w-4xl flex-col items-center gap-4 text-center">
          <p className="text-xs uppercase tracking-[0.34em] text-slate-500">
            Heap / Greedy / Feasibility Frontier
          </p>
          <h1 className="text-4xl font-semibold tracking-tight md:text-6xl">
            <span className="text-cyan-400 text-glow-cyan">IPO</span>
          </h1>
          <p className="max-w-3xl text-sm leading-7 text-slate-400 md:text-base">
            Watch capital unlock projects in sorted order, then let a max-heap
            greedily choose the most profitable affordable move at each round.
          </p>
        </header>
          ...
          <div className="glass-card p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  value={profitsInput}
                  onChange={(event) => setProfitsInput(event.target.value)}
                  className="input-field mt-2"
                  placeholder="1, 2, 3"
                />
              </div>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400">
              <span>
                Profit and capital arrays are paired by index and truncated to the shorter length.
              </span>
              <button
                onClick={() =>
                  runWithValues(
          ...
          >
            ...
            <div className="rounded-xl bg-slate-950/50 p-4 font-mono text-base">
              <span className={step.done ? "text-emerald-300" : "text-cyan-300"}>
                finalCapital = {formatCapital(liveAnswer)}
              </span>
            </div>
