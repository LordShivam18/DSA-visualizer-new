...
import Controls from "../../../components/backtracking/letter-combinations-of-a-phone-number/Controls";
...
import {
  formatResultList,
  generateTrace,
  type LetterCombinationsTraceStep,
} from "../../../components/backtracking/letter-combinations-of-a-phone-number/generateTrace";

const defaultInput = "23";
...
const presets = [
  ...
  },
  {
    name: "Three Digits",
    value: "279",
    output: "48 combinations",
  },
] as const;

export default function LetterCombinationsPage() {
  const [input, setInput] = useState(defaultInput);
  const [trace, setTrace] = useState<LetterCombinationsTraceStep[]>(() =>
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6">
        ...
        <header className="mx-auto flex w-full max-w-4xl flex-col items-center gap-4 text-center">
          <p className="text-xs uppercase tracking-[0.34em] text-slate-500">
            Backtracking / Cartesian Product / Phone Keypad DFS
          </p>
          ...
          <p className="max-w-3xl text-sm leading-7 text-slate-400 md:text-base">
            Choose one mapped letter per digit, recurse to the next position,
            and backtrack cleanly so every phone-word branch gets explored once.
          </p>
        </header>
        ...
        <div className="mx-auto w-full max-w-4xl">
          <div className="glass-card p-5">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400">
              <span>
                Only digits 2 through 9 are used. Other characters are ignored safely.
              </span>
        ...
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,0.92fr)]">
          <section className="space-y-5">
            ...
            <LetterCombinationsWorkbench step={step} />
          ...
          >
            ...
            <div className="rounded-xl bg-slate-950/50 p-4 font-mono text-base">
              <span className={step.done ? "text-emerald-300" : "text-cyan-300"}>
                letterCombinations(...) = {formatResultList(step.state.results)}
              </span>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-400">
              <span>
                ...
                <span className="font-mono text-slate-200">
                  O(number of generated strings)
                </span>
              </span>
              <span>
                Last saved word:{" "}
                <span className="font-mono text-slate-200">
                  {step.state.results[step.state.results.length - 1] ?? "none"}
                </span>
              </span>
            </div>
          </div>
