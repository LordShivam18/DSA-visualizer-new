...
import Controls from "../../../components/binary-tree-bfs/average-of-levels-in-binary-tree/Controls";
...
import {
  formatAverageArray,
  generateTrace,
  type AverageTraceStep,
} from "../../../components/binary-tree-bfs/average-of-levels-in-binary-tree/generateTrace";
...
export default function AverageOfLevelsInBinaryTreePage() {
  ...
  const [trace, setTrace] = useState<AverageTraceStep[]>(() =>
  ...
  const progressLabel = step.done
    ? `averageOfLevels = ${formatAverageArray(step.state.resultAverages)}`
    : `averages so far = ${formatAverageArray(step.state.resultAverages)}`;
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6">
        ...
        <header className="mx-auto flex w-full max-w-4xl flex-col items-center gap-4 text-center">
          <p className="text-xs uppercase tracking-[0.34em] text-slate-500">
            Binary Tree BFS / Level Statistics
          </p>
          <h1 className="text-4xl font-semibold tracking-tight md:text-6xl">
            Average of Levels{" "}
            <span className="text-cyan-400 text-glow-cyan">In Binary Tree</span>
          </h1>
          <p className="max-w-3xl text-sm leading-7 text-slate-400 md:text-base">
            This visualizer teaches the queue-based BFS solution for LeetCode 637.
            Each level is isolated, its values are accumulated into a running sum,
            and the final average is committed only after the whole row is processed.
          </p>

          <div className="flex flex-wrap justify-center gap-3">
            <div className="rounded-full border border-slate-700/80 bg-slate-950/70 px-4 py-2 text-xs text-slate-300">
              Example Input{" "}
              <span className="font-mono text-cyan-200">[3,9,20,null,null,15,7]</span>
            </div>
            <div className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-4 py-2 text-xs text-emerald-100">
              Example Output <span className="font-mono">[3.00000, 14.50000, 11.00000]</span>
            </div>
        ...
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,0.92fr)]">
          <section className="space-y-5">
            ...
            <AverageLevelsWorkbench step={step} />
