...
import Controls from "../../../components/binary-tree-bfs/binary-tree-right-side-view/Controls";
...
} from "../../../components/binary-tree-bfs/binary-tree-right-side-view/generateTrace";
...
const defaultTree = "[1,2,3,null,5,null,4]";
...
const presets: PresetExample[] = [
  ...
  },
  {
    name: "Wide Tree",
    values: {
      root: "[1,2,3,4,5,6,7,null,null,null,8]",
    },
  },
  {
    name: "Empty Tree",
...
export default function BinaryTreeRightSideViewPage() {
  ...
  const [trace, setTrace] = useState<RightViewTraceStep[]>(() =>
  ...
  const progressLabel = step.done
    ? `rightSideView = ${formatRightView(step.state.resultValues)}`
    : `visible so far = ${formatRightView(step.state.resultValues)}`;

  return (
    <div className="relative min-h-screen overflow-hidden grid-pattern bg-[#030611] text-slate-50">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.12),transparent_28%),radial-gradient(circle_at_top_right,rgba(96,165,250,0.08),transparent_24%),linear-gradient(180deg,rgba(3,6,17,0.94),rgba(3,6,17,1))]" />
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6">
        ...
        <header className="mx-auto flex w-full max-w-4xl flex-col items-center gap-4 text-center">
          <p className="text-xs uppercase tracking-[0.34em] text-slate-500">
            Binary Tree BFS / Right Side View
          </p>
          <h1 className="text-4xl font-semibold tracking-tight md:text-6xl">
            Binary Tree{" "}
            <span className="text-cyan-400 text-glow-cyan">Right Side View</span>
          </h1>
          <p className="max-w-3xl text-sm leading-7 text-slate-400 md:text-base">
            This visualizer teaches the classic queue-based BFS solution for LeetCode 199.
            Each step freezes one level, processes nodes from left to right, and captures the
            last node popped because that is the one visible from the right side.
          </p>

          <div className="flex flex-wrap justify-center gap-3">
            <div className="rounded-full border border-slate-700/80 bg-slate-950/70 px-4 py-2 text-xs text-slate-300">
              Example Input{" "}
              <span className="font-mono text-cyan-200">[1,2,3,null,5,null,4]</span>
            </div>
            <div className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-4 py-2 text-xs text-emerald-100">
              Example Output <span className="font-mono">[1,3,4]</span>
            </div>
        ...
        <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,0.92fr)]">
          <section className="space-y-5">
            ...
            <BinaryTreeRightSideViewWorkbench step={step} />
