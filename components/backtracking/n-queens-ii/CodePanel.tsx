import CodeBlock from "../shared/CodeBlock";
import PanelShell from "../shared/PanelShell";
import type { NQueensTraceStep } from "./generateTrace";

type Props = {
  step: NQueensTraceStep;
};

const codeLines = [
  "int totalNQueens(int n) {",
  "    int answer = 0;",
  "    vector<int> columns(n, -1);",
  "    unordered_set<int> usedCols, diagDown, diagUp;",
  "    function<void(int)> dfs = [&](int row) {",
  "        if (row == n) {",
  "            ++answer;",
  "            return;",
  "        }",
  "        for (int col = 0; col < n; ++col) {",
  "            if (usedCols.count(col) || diagDown.count(row - col) || diagUp.count(row + col)) continue;",
  "            columns[row] = col;",
  "            usedCols.insert(col);",
  "            diagDown.insert(row - col);",
  "            diagUp.insert(row + col);",
  "            dfs(row + 1);",
  "            usedCols.erase(col);",
  "            diagDown.erase(row - col);",
  "            diagUp.erase(row + col);",
  "            columns[row] = -1;",
  "        }",
  "    };",
  "    dfs(0);",
  "    return answer;",
  "}",
];

export default function CodePanel({ step }: Props) {
  return (
    <PanelShell
      title="Code Panel"
      subtitle="The highlights follow the constant-time conflict checks and the exact place where queens are committed and removed."
      accent="cyan"
    >
      <div className="flex flex-wrap gap-2 text-xs">
        <span className="rounded-full border border-cyan-400/30 bg-cyan-500/10 px-3 py-1 text-cyan-100">
          Time: exponential backtracking
        </span>
        <span className="rounded-full border border-violet-400/30 bg-violet-500/10 px-3 py-1 text-violet-100">
          Space: O(n)
        </span>
        <span className="rounded-full border border-amber-400/30 bg-amber-500/10 px-3 py-1 text-amber-100">
          Active Lines: {step.codeLines.join(", ")}
        </span>
      </div>

      <div className="mt-5">
        <CodeBlock codeLines={codeLines} activeLines={step.codeLines} />
      </div>

      <div className="mt-4 rounded-[1.15rem] border border-slate-800/80 bg-slate-950/55 px-4 py-3 text-sm text-slate-300">
        <span className="text-slate-500">Why these lines:</span>{" "}
        {step.explanationExpert}
      </div>
    </PanelShell>
  );
}
