...
import type {
  CombinationsActionKind,
  CombinationsTraceStep,
} from "./generateTrace";
...
const actionTone: Record<CombinationsActionKind, string> = {
  ...
  "choose-number": "border-cyan-400/40 bg-cyan-500/10 text-cyan-200",
...
export default function TracePanel({ step }: Props) {
  return (
    <PanelShell
      title="Trace Panel"
      subtitle="Each step records one subset decision or one undo."
      accent="amber"
    >
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <StatCard label="Start" value={step.pointers.start ?? "done"} tone="violet" />
        <StatCard
          label="Candidate"
