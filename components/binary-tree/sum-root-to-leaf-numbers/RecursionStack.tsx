import type { SumRootStackFrame } from "./generateTrace";

type Props = {
  stack: SumRootStackFrame[];
};

const toneClass: Record<SumRootStackFrame["status"], string> = {
  active:
...
const relationTone: Record<SumRootStackFrame["relation"], string> = {
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-xs leading-6 text-slate-400">
            The top card is the active DFS call and the number built so far for
            that path.
          </p>
        </div>
