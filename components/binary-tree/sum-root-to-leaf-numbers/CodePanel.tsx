...
const codeLines = [
  ...
  "    if (!node) return 0;",
  "    int nextNumber = currentNumber * 10 + node->val;",
  "    if (!node->left && !node->right) {",
  "        return nextNumber;",
  "    }",
  "    int leftSum = dfs(node->left, nextNumber);",
  "    int rightSum = dfs(node->right, nextNumber);",
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">
            Highlighted lines track the recursive construction and aggregation
            of root-to-leaf numbers.
          </p>
        </div>
      </div>
