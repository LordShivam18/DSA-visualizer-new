import type { MaxPathTreeNode } from "./generateTrace";
...
export default function PathGainStrip({
  title,
  ids,
  nodes,
  accent,
      ...
      {values.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-slate-500">Path Sum:</span>
            <span className={`font-mono text-lg ${palette.total}`}>{total}</span>
          </div>
        </div>
      )}
    </div>
