...
const codeLines = [
  ...
  "    if (!node) return 0;",
  "    int leftGain = maxGain(node->left, best);",
  "    int rightGain = maxGain(node->right, best);",
  "    int useLeft = max(leftGain, 0);",
  "    int useRight = max(rightGain, 0);",
  "    int throughNode = node->val + useLeft + useRight;",
  "    best = max(best, throughNode);",
  "    return node->val + max(useLeft, useRight);",
  "}",
];
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">
            Highlighted lines track the postorder gain propagation and global-best update.
          </p>
        </div>
      </div>
