...
export default function DarkControls({
  stepIndex,
  totalSteps,
  mode,
  onModeChange,
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={onPrev}
            disabled={!canPrev}
            className="rounded-xl border border-slate-700/80 bg-slate-950/70 px-5 py-2 text-sm font-medium text-slate-200 transition-all hover:border-violet-400/50 hover:text-violet-200 disabled:cursor-not-allowed disabled:border-slate-800 disabled:text-slate-500"
          >
            Prev Step
          </button>
          <button
            onClick={onNext}
            disabled={!canNext}
            className="rounded-xl bg-cyan-500 px-5 py-2 text-sm font-semibold text-slate-950 shadow-[0_0_26px_rgba(34,211,238,0.38)] transition-all hover:-translate-y-0.5 hover:bg-cyan-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400 disabled:shadow-none"
          >
            Next Step
          </button>
          <button
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2 rounded-full border border-slate-800 bg-slate-900/70 p-1">
            ...
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-xs transition-all ${
                mode === "expert"
                  ? "bg-violet-500 text-white shadow-[0_0_16px_rgba(167,139,250,0.6)]"
                  : "text-slate-300 hover:bg-slate-800"
              }`}
            >
              Expert
