...
function buildPositionMap(step: MinDiffTraceStep) {
...
export default function MinimumDifferenceWorkbench({
  step,
}: {
  ...
  const bestPairSet = new Set(step.pointers.bestPairIds ?? []);
    ...
    <section className="glass-card overflow-hidden p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold text-slate-50">
            Compare neighboring sorted values
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
            Cyan marks the active node, amber marks the saved stack frontier,
            yellow marks the previous inorder node, rose marks the comparison
            pair, and green marks the best pair found so far.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          <span className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-emerald-100">
            Best {formatDiff(step.state.bestDiff)}
          </span>
        </div>
      </div>
        ...
        >
          {step.state.nodes.length === 0 ? (
            <div className="flex h-full items-center justify-center text-sm text-slate-500">
              Empty BST. No pair can be compared.
            </div>
          ) : (
            <>
              <svg
                  ...
                  return (
                    <g key={`edges-${node.id}`}>
                      {[node.leftId, node.rightId].map((childId) => {
                        ...
                        const bestEdge =
                          bestPairSet.has(node.id) && bestPairSet.has(childId);
                        ...
                        return (
                          <line
                            ...
                            y2={target.y}
                            stroke={
                              bestEdge ? "#34d399" : activeEdge ? "#22d3ee" : "#334155"
                            }
                            strokeWidth={bestEdge || activeEdge ? 3 : 2}
                            opacity={bestEdge || activeEdge ? 0.9 : 0.58}
                          />
                        );
                      })}
                    </g>
              ...
              {step.state.nodes.map((node) => {
                ...
                const isPrev = step.pointers.prevId === node.id;
                const isCompare = step.pointers.compareWithId === node.id || isFocus;
                const isBest = bestPairSet.has(node.id);
                ...
                const toneClass =
                  isBest
                    ? "border-emerald-400/80 bg-emerald-500/16 text-emerald-50 shadow-[0_0_30px_rgba(52,211,153,0.24)]"
                    : isFocus
                    ? "border-cyan-400/80 bg-cyan-500/16 text-cyan-50 shadow-[0_0_30px_rgba(34,211,238,0.26)]"
                    : isCompare
                    ? "border-rose-400/70 bg-rose-500/12 text-rose-50 shadow-[0_0_26px_rgba(251,113,133,0.18)]"
                    : isPrev
                    ? "border-yellow-400/70 bg-yellow-500/12 text-yellow-50 shadow-[0_0_26px_rgba(250,204,21,0.18)]"
                    : isStacked
                    ? "border-amber-400/55 bg-amber-500/10 text-amber-50 shadow-[0_0_22px_rgba(251,191,36,0.12)]"
                    : isVisited
                    ? "border-emerald-400/45 bg-emerald-500/8 text-emerald-50 shadow-[0_0_20px_rgba(52,211,153,0.1)]"
                ...
                return (
                  <TreeNodeCard
                    ...
                    dimmed={!isFocus && !isStacked && !isVisited && step.state.nodes.length > 1}
                  />
                );
              })}
            </>
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-2">
        <StackStrip
          ...
          helperText="The top of this stack is the next ancestor to visit."
        />
        <InorderStrip
