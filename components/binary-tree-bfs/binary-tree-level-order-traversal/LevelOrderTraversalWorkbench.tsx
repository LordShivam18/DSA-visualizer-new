...
function buildPositionMap(step: LevelOrderTraceStep) {
...
export default function LevelOrderTraversalWorkbench({
  step,
}: {
    ...
    <section className="glass-card overflow-hidden p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold text-slate-50">
            Build one answer row per queue frontier
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
            Cyan tracks the node being popped, yellow marks the active row,
            purple shows children queued for the next row, rose marks values
            already copied into the current vector, and green marks committed
            output levels.
          </p>
        </div>
        ...
        >
          {step.state.nodes.length === 0 ? (
            <div className="flex h-full items-center justify-center text-sm text-slate-500">
              Empty tree. No BFS levels exist.
            </div>
          ) : (
            <>
              <svg
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        <QueueStrip
          ...
          helperText="Only these nodes still belong to the row currently being copied."
        />
        <QueueStrip
          title="Next Frontier"
