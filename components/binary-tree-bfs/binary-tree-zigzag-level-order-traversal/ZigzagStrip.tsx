...
export default function ZigzagStrip({
  title,
  summaries,
  nodes,
  emptyLabel,
      ...
      {summaries.length === 0 ? (
        ...
        <div className="mt-4 space-y-3">
          {summaries.map((summary) => {
            const bfsValues = summary.nodeIds.map(
            ...
            return (
              <div
                key={`zigzag-level-${summary.level}`}
                className={[
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full border border-yellow-400/30 bg-yellow-500/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.16em] text-yellow-100">
                    {formatDirection(summary.direction)}
                  </span>
