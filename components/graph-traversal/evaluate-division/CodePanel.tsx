...
const codeLines = [
  ...
  "        if (!graph.count(src) || !graph.count(dst)) { answer.push_back(-1.0); continue; }",
  "        if (src == dst) { answer.push_back(1.0); continue; }",
  ...
  "            st.pop();",
  "            if (node == dst) { resolved = ratio; break; }",
  "            for (auto& [next, weight] : graph[node]) {",
  "                if (seen.count(next)) continue;",
  "                seen.insert(next);",
  "                st.push({next, ratio * weight});",
  "            }",
  "        }",
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">
            Highlighted lines follow graph construction, query guards, and DFS stack updates.
          </p>
        </div>
      </div>
