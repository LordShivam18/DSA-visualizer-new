...
function buildLevels(heap: Project[]) {
...
export default function ProjectHeapPanel({
  heap,
  heapIndex,
  compareIndex,
  chosenProjectId,
    ...
    <div className="rounded-[1.3rem] border border-slate-800/80 bg-[#050916] p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-slate-50">
            Root = best profit available this round
          </h3>
        </div>
      ...
      {heap.length === 0 ? (
        <div className="mt-5 rounded-[1.15rem] border border-dashed border-slate-700/80 px-4 py-10 text-center text-sm text-slate-500">
          No affordable project is waiting in the heap.
        </div>
      ) : (
        <>
          <div className="mt-5 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {heap.map((project, index) => {
              let toneClass =
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-[11px] text-slate-400">
                      need {project.capital}
                    </span>
                  </div>
                  <div className="mt-2 text-2xl font-semibold">{project.profit}</div>
                </div>
              );
            })}
          </div>
            ...
            <div className="mt-4 space-y-4">
              {levels.map((level, levelIndex) => {
                ...
                return (
                  ...
                  >
                    {level.map((project, offset) => {
                      const index = levelStart + offset;
                      const isChosen = project.id === chosenProjectId;
