import type { GeneQueueEntry } from "./generateTrace";
...
export default function MutationQueue({ queue, activeGene }: Props) {
  return (
    <div className="rounded-[1.2rem] border border-slate-800/80 bg-slate-950/55 p-4">
      ...
      {queue.length === 0 ? (
        <div className="mt-4 rounded-xl border border-dashed border-slate-700/80 px-3 py-5 text-sm text-slate-500">
          The mutation queue is empty right now.
        </div>
      ) : (
        <div className="mt-4 space-y-2">
          {queue.map((entry, index) => {
            const isActive = entry.gene === activeGene;

            return (
              <div
                key={`${entry.gene}-${index}`}
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="font-mono text-sm">{entry.mutations}</span>
                </div>
              </div>
            );
          })}
